...
class NestedSetsQueryBehavior extends Behavior
{
    ...
    public function children($id, $depth = null)
    {
        
        $model = eval('return \\'. $this->owner->modelClass . "::findOne(['id'=>$id]);");
        $condition = [
            'and',
            ['>', $model->leftAttribute, $model->getAttribute($model->leftAttribute)],
            ['<', $model->rightAttribute, $model->getAttribute($model->rightAttribute)],
        ];

        if ($depth !== null) {
            $condition[] = ['<=', $model->depthAttribute, $model->getAttribute($model->depthAttribute) + $depth];
        }
        ...
        return $this->owner->andWhere($condition);//->addOrderBy([$model->leftAttribute => SORT_ASC]);
